...
CREATE INDEX idx_workflows_contact ON workflows(contact_id);
...
│   │   ├── workflows/          # Workflow management (was "workflow_executions")
...
│   │   ├── workflows.ts        # (was workflow_executions.ts)
│   │   ├── contacts.ts
│   │   ├── tasks.ts
│   │   ├── activity.ts
│   │   └── settings.ts
...
- [ ] Workflow orchestrates: create contact → create workflow execution → send notifications → create tasks
...
2. ✅ Formstack webhook triggers workflows via Temporal
3. ✅ Users can build workflow definitions with linear step builder
4. ✅ Users can trigger and monitor workflow executions
5. ✅ Tasks can be created, assigned (user or role), and tracked via Kanban
6. ✅ Approval tasks work with approve/reject pattern
7. ✅ Dashboard shows pipeline overview
