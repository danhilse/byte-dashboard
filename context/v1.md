...
| Workflow Engine | Temporal.io | Durable workflow orchestration (TypeScript SDK) |
...
**Byte Dashboard is a workflow automation platform** that orchestrates multi-step processes involving external integrations and long-running operations. Unlike simple CRUD applications, workflows can span hours, days, or weeks while waiting for external events.
...
- Example: `application-review.ts`, `background-check.ts`
...
- **Application Data**: PostgreSQL (via Drizzle) - contacts, applications, tasks
...
The `workflow_templates` table stores UI configuration (status badges, colors, labels), while Temporal handles the actual execution, retries, and durability.
...
│   │   ├── workflows/          # Workflow management (was "applications")
...
│   │   ├── workflows.ts        # (was applications.ts)
│   │   ├── contacts.ts
│   │   ├── tasks.ts
│   │   ├── activity.ts
│   │   └── settings.ts
...
2. ✅ Formstack webhook creates workflows automatically
3. ✅ Users can view/manage workflows and contacts
4. ✅ Documents can be uploaded and attached to workflows
...
- [ ] Temporal workflow triggered by webhook
- [ ] Workflow orchestrates contact/application creation
...
**Workflows are managed by Temporal**, not simple status transitions. The `workflow_templates` table stores UI metadata (status labels, colors, badges for the frontend), while Temporal handles execution, retries, and durability.
...
- **UI Metadata**: `workflow_templates` table still defines statuses for badges/filters in frontend
...
- Focus on the core loop: Form → Contact → Workflow → Tasks → Documents
